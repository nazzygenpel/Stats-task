...
async function main() {
  const movieStore = new MovieStore('New Store in the City');
  await movieStore.initialize();

  console.log('\n****Adding new movies to the store****\n');
  movieStore.addMovie({ title: 'Adire', totalNumber: 12 });
  movieStore.addMovie({ title: 'Sound Of Music', totalNumber: 5 });

  console.log('Show available movies\n');
  console.log(movieStore.getMovies());

  console.log('\n****Renting movies from the store****\n');
  const rented1 = movieStore.rent('Adire');
  console.log('Successfully rented', rented1);

  const rented2 = movieStore.rent('Sound Of Music');
  ...
  const returnedMovies = movieStore.returnMovie('Adire');
